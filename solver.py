...
class TSPSolver(Solver):
    @timer
    def solve(self, problem: TSP, solution_protocol: SolutionProtocol, init_sol: TSPSolution = None, verbose = True) -> Solution:
        '''Solve the system and store in the class'''
        if init_sol is None:
            init_sol = TSPSolution(problem.N)
        return solution_protocol.search(problem, init_sol, verbose)
